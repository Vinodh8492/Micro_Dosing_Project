...
const Orders = () => {
  ...
  const fetchOrders = async () => {
    try {
      const res = await axios.get('http://127.0.0.1:5000/api/production_orders');
  ...
  const fetchRecipes = async () => {
    try {
      const res = await axios.get('http://127.0.0.1:5000/api/recipes');
      setRecipes(res.data);
    } catch (err) {
  ...
  const handleSubmit = async () => {
    try {
      const config = {
        withCredentials: true, // ✅ This ensures cookies (like JWT) are sent with the request
      };
      ...
      if (editingOrder) {
        ...
        await axios.post(
          ...
          formData,
          config // ✅ Pass the config here too
        );
      }
  ...
  const handleDelete = async (orderId) => {
    ...
    try {
      await axios.delete(`http://127.0.0.1:5000/api/production_orders/${orderId}`);
      await fetchOrders();
    } catch (error) {
